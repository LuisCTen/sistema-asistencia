...
AS
   ...
   PROCEDURE sp_registrar_salida( p_id_usuario  IN usuario.id_usuario %TYPE ,
                                  p_resultado  OUT VARCHAR2                );
   -- Función para obtener la fecha de asistencia (TRUNC(SYSDATE))
   FUNCTION obtener_fecha_asistencia RETURN DATE;
...
--Body
    ...
    --
       ...
       BEGIN
          ...
          IF l_existe>0 THEN
             --
             p_resultado :='ERROR :Ya existe una asistencia regitrada para hoy';
             RETURN;
             --
          END IF;
          --
       ...
       BEGIN 
         --Busca si el usuario en ese mismo día tiene generado un registro de entrada
          BEGIN
          SELECT a.id_asistencia,a.hora_salida 
          INTO l_id_asistencia, l_hora_salida
          FROM asistencia a
          WHERE  a.id_usuario     = p_id_usuario
             AND a.fec_asistencia = l_fec_actual;
             --AND a.hora_salida    IS NULL;--Aqui verifica que no haya marcado salida
          EXCEPTION WHEN NO_DATA_FOUND THEN
             p_resultado := 'ERROR: No haz marcado asistencia de entrada hoy.';
             RETURN;
          END;
          --
          --Validamos si ya tenia salida marcada
          IF l_hora_salida IS NOT NULL THEN
             --
             p_resultado :='ERROR:Ya registraste tu salida para el dia de hoy.';
          ...
          COMMIT;
          p_resultado:='OK: Se registro salida con exito';
    
       EXCEPTION WHEN OTHERS THEN
          ROLLBACK;
          p_resultado:='Error crítico: '||SQLERRM;
